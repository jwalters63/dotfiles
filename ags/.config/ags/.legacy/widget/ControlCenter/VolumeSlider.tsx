...
async function getVolume(): Promise<number> {
  try {
    const out = await execAsync(["pactl", "get-sink-volume", "@DEFAULT_SINK@"]).catch(() => "")
...
async function getMuted(): Promise<boolean> {
  try {
    const out = await execAsync(["pactl", "get-sink-mute", "@DEFAULT_SINK@"]).catch(() => "")
...
const volIcon = createPoll("audio-volume-low-symbolic", 2000, async () => {
  try {
    const mute = (await execAsync(["pactl", "get-sink-mute", "@DEFAULT_SINK@"]).catch(() => "")).includes("yes")
    return mute ? "audio-volume-muted-symbolic" : "audio-volume-low-symbolic"
  } catch {
  ...
  return (
    <box class="slider-module" orientation={Gtk.Orientation.VERTICAL} widthRequest={312} heightRequest={72}>
      <box css="padding: 10px 14px 9px 14px;" orientation={Gtk.Orientation.VERTICAL} spacing={6} hexpand vexpand>
        <label label="Sound" class="slider-label" halign={Gtk.Align.START} />
          ...
          $={(self: Astal.Slider) => {
            ...
            self.connect("value-changed", () => {
              if (!fromPoll) {
                ...
                execAsync(["pactl", "set-sink-volume", "@DEFAULT_SINK@", `${pct}%`]).catch(() => {})
              }
            })
          }}
        />

          <image iconName="audio-volume-high-symbolic" pixelSize={16} class="slider-icon" />
        </box>
      </box>
    </box>
  )
